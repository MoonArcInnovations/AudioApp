...
- What happens when startup or backend initialization succeeds only partially?
- How does the system handle missing role, missing verification state, or malformed remote profile data?
- What happens when the user is offline during a flow that also depends on cloud state?
- What happens when a user attempts to access a route or action outside their role?
...
- **FR-008**: System MUST define how role-based access, auditability, and failure visibility apply to the feature when relevant.
